...
const getMenu = async (req, res) => {
    try {
        ...
        if (!sedeId || isNaN(Number(sedeId))) {
            return res.status(400).json({ error: 'El par�metro idempresa es inv�lido.' });
        }
        ...
        if (rows.length === 0) {
            return res.status(404).json({ error: `No se encontraron men�s para la empresa con ID ${sedeId}.` });
        }

        return res.json(rows);
    } catch (error) {
        console.error('Error al obtener men�s:', error);
        return res.status(500).json({ error: 'Error interno del servidor al obtener los men�s.' });
    }
};
...
const llenarDomicilios = async (req, res) => {
    try {
        ...
        // Consultar la direcci�n del usuario desde la tabla Usuario
        ...
        // Obtener el ID del domicilio reci�n creado
        const domicilioId = result.insertId;

        // Insertar los men�s en la tabla ComidaDomicilio
...
const getConsecutivo = async (req, res) => {
    try {
        ...
        // Consulta SQL para obtener el n�mero de domicilio m�s alto para la sede
        const [rows] = await pool.execute(
...
const getDetalleDomicilio = async (req, res) => {
    try {
        ...
        // Consulta SQL para obtener los men�s asociados al domicilio
        ...
        // Combinar los detalles del domicilio con los men�s
...
const cambiarEstadoDomicilio = async (req, res) => {
    try {
        ...
        // Validar que el nuevo estado est� dentro del rango permitido (1-3)
